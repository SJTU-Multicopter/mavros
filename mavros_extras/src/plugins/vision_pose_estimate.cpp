...
private:
	...
	void odom_cb(const nav_msgs::Odometry &msg) {
		...
		{
			pos_old = pos;
			pos_rec.header = pos.header;
			pos_rec.pose.position.x = 0.0;
			pos_rec.pose.position.y = 0.0;			
			pos_rec.pose.position.z = 0.0;
			init = false;
		}
		//chg, asume 10Hz, 3m/s
		if(fabs(pos.pose.position.x - pos_old.pose.position.x) > 0.3 || fabs(pos.pose.position.y - pos_old.pose.position.y) > 0.3 || fabs(pos.pose.position.z - pos_old.pose.position.z) > 0.3)  
		{
			pos_rec = pos_old;			
		}

		pos.pose.position.x += pos_rec.pose.position.x;
		pos.pose.position.y += pos_rec.pose.position.y;			
		pos.pose.position.z += pos_rec.pose.position.z;
		...
		geometry_msgs::PoseStamped &req = pos;
